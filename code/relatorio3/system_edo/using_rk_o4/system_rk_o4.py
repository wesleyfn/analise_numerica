...
def system_edo_rk_o4(exprs, list_y0, x0, h, n_h):
    ...
    list_x = list(np.linspace(x0, x0 + n_h * h, n_h + 1))
...
def run():
    FILE_NAME = 'input.txt'
    exprs, list_y, x0, h, n_h = read_file(FILE_NAME)

    if exprs is not None:
        x_sol, y_sol = system_edo_rk_o4(exprs, list_y, x0, h, n_h)
